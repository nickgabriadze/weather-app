...
const Content = () => {
    ...
    const currentHourData = () => {
        if (!loadingPhase) {
            ...
            const currentData = weatherData.days[0].hours.filter((eachObject: any) => {
                return new Date().getHours() === Number(eachObject.datetime.slice(0, 2));
            });
    ...
    let formattedTime = `${new Date().getHours() > 9 ? new Date().getHours() : "0" + new Date().getHours()}:${new Date().getMinutes() > 9 ? new Date().getMinutes() : "0" + new Date().getMinutes()}`;

    return (
        <>
            {loadingPhase ? <p>GETTING THE DATA</p> :
                <div className={contentstyle["content-wrapper"]}>

                    <div className={contentstyle["header"]}>
                        <div className={contentstyle["header-content"]}>
                            <h2 className={contentstyle['day-txt']}>{`${weekDays[new Date().getDay()]}, ${formattedTime}`}</h2>

                            <div className={contentstyle["city-icon"]}>
                                <h1 className={contentstyle["city-txt"]}> {city} </h1>
                                <h1 className={contentstyle["temperature"]}>{Math.round(currentHourData().temp)}&deg;</h1>
                                <Image src={icons[currentHourData().icon]} height={60} width={60} alt={currentHourData().icon} />
                            </div>
                        </div>

                        <div className={contentstyle["current-weather-info"]}>
                            <Image src={windPowerIcon} height={60} width={60} alt={"Wind Power icon"} />
                            <h1>{currentHourData().windspeed}km/ph</h1>
                        </div>

                        <div className={contentstyle["current-weather-info"]}>
                            <Image src={precipitationIcon} height={60} width={60} alt={"Water Drop icon"} />
                        ...
                        <div className={contentstyle["current-weather-info-desc"]}>
                            <Image src={descriptionIcon} height={60} width={60} alt={"Description icon"} />
                    ...
                    <div className={contentstyle["hourly"]}>
                        ...
                        <div className={contentstyle['hour-boxes']}>
                            {
                                weatherData.days[0].hours.map((eachObject: any, index: number) => {

                                    return (
                                        <div key={index} className={contentstyle['each-hour-box']}>
                                                <h1>{eachObject.datetime.slice(0, 5)}</h1>
                                                <Image src={icons[eachObject.icon]} alt={eachObject.icon} width={70} height={70}/>
                                                <h1 className={contentstyle['temperature']}>{Math.round(eachObject.temp)}&deg;</h1>
